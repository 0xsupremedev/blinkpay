...
export async function initMerchantOnChain({ merchantAuthority, displayName }: { merchantAuthority: string; displayName?: string | null }) {
...
export async function buildPayTransaction({ payer, merchantOwner, mint, amount, receiptId }: { payer: string; merchantOwner: string; mint: string; amount: number; receiptId: string; }) {
...
export async function buildPayWithSplitTransaction({ payer, merchantOwner, mint, amount, receiptId, feeBps, platform }: { payer: string; merchantOwner: string; mint: string; amount: number; receiptId: string; feeBps: number; platform: string; }) {
	const { program, programId } = getProgram();
	const payerPk = new PublicKey(payer);
	const merchantOwnerPk = new PublicKey(merchantOwner);
	const mintPk = new PublicKey(mint);
...
export async function buildPartialRefundTransaction({ merchantAuthority, payer, mint, amount, receipt }: { merchantAuthority: string; payer: string; mint: string; amount: number; receipt: string; }) {
	const { program, programId } = getProgram();
