...
export async function generateBlinkLogin({ sessionId, pubkey }: { sessionId: string; pubkey: string }) {
	const url = `${FRONTEND_BASE}/login?sessionId=${encodeURIComponent(sessionId)}&pubkey=${encodeURIComponent(pubkey)}`;
	const qrDataUrl = await QRCode.toDataURL(url);
	return { url, qrDataUrl };
}

export async function generatePaymentBlink({ merchantOwner, amount, mint, requestId }: { merchantOwner: string; amount: number; mint: string; requestId: string }) {
