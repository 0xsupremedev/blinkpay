...
import Joi from "joi";
import { generatePaymentBlink } from "../services/blink";
import { db } from "../services/supabase";
import { buildPayTransaction, buildPayWithSplitTransaction, buildPartialRefundTransaction } from "../services/solana";
...
router.post("/create-blink", async (req, res) => {
	try {
		...
		if (idem) {
			if (seen.has(idem)) return res.status(200).json({ status: "duplicate" });
			seen.add(idem);
		}
		const { value, error } = schema.validate(req.body);
		if (error) return res.status(400).json({ error: error.message });
		const { merchantOwner, amount, mint, requestId, description, payer, returnTx } = value as any;
...
router.post("/build-pay-split", async (req, res) => {
	try {
		...
		if (error) return res.status(400).json({ error: error.message });
		const { merchantOwner, amount, mint, payer, feeBps, platform, receiptId } = value as any;
...
router.post("/build-refund-partial", async (req, res) => {
	try {
		...
		if (error) return res.status(400).json({ error: error.message });
		const tx = await buildPartialRefundTransaction(value as any);
		res.json(tx);
	} catch (e: any) {
		res.status(500).json({ error: String(e) });
	}
